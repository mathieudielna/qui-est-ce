...
class PcaController extends AbstractController
{
    /**
     * @Route("/app/pca", name="pca_list")
     */
    public function indexpca(ActiviteRepository $repoactivite, MetierRepository $repometier, ProcessusRepository $repoprocessus, CriticiteRepository $repocriticite, ApplicationRepository $repoapplication, AppConnectActiviteRepository $repoappconnect, PcaEvenementRepository $repopcaevenement, SystemeRepository $reposysteme)
    {	    
        $confo = 'SI';
		...
		else
	    {
	   	...
	   	$criticites = $repocriticite->findByCustomer($this->getUser()->getCustomer());
	   	$applications = $repoapplication->findByCustomer($this->getUser()->getCustomer());
	   	$evenements = $repopcaevenement->findByCustomer($this->getUser()->getCustomer());
	   	$appconnects = $repoappconnect->findAll();
        return $this->render('blog/pca_list.html.twig', [
            ...
            'systemes' => $systemes,
            'appConnectActivites' => $appconnects,
            'conformite' => $confo
        ]);
    }
}
