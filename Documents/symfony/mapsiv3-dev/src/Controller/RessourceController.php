...
class RessourceController extends AbstractController
{
     ...
     /**
     * @Route("/app/{domid}/ressource", name="ressource")
     * @internal param Registry $workflows
     * @param Ressource $ressource
     */
    public function indexrisque(Request $request, ressourceRepository $reporessource, $domid)
    {   
        if ($request->isXmlHttpRequest()) {
            ...
            {
                $ressources = $reporessource->findAllCustomerUser($this->getUser()->getCustomer(),$this->getUser()->getRessource());
            }
            ...
            foreach($ressources as $ressource) {
                if ($ressource->getResponsable()) {$responsable=$ressource->getResponsable()->getFirstname().' '.$responsable=$ressource->getResponsable()->getLastname();}
                else {$responsable="";}
                if ($ressource->getSuppleant()) {$suppleant=$ressource->getSuppleant()->getFirstname().' '.$suppleant=$ressource->getSuppleant()->getLastname();}
                else {$suppleant="";}
                $data[] = array(
                    'select' => '',
                    'statut' => '',
                    'designation' => '<a onclick="crossEntity('.$ressource->getId().',\'ressource\')">'.$ressource->getDesignation().'</a>',
        ...
        {
            return $this->redirectToRoute('switch', ['domid' => $domid,'entite' => 'ressource', 'domaine'=>'', 'trans'=>'', 'tdb'=>'']);
        }  
    }

    /** 
    * @route("/app/ressource/ajaxcreate", name="ressource_ajaxcreate", methods={"POST", "GET"}) 
    * @route("/app/ressource/ajaxedit/{id}/{entite}", name="ressource_ajaxedit", methods={"POST", "GET"})
    */ 
    public function ajaxressource(Ressource $ressource = null, Activite $activite = null, Log $log = null, Request $request, ObjectManager $manager){
        if(!$ressource) {
            $ressource = new Ressource();
            $ressource->setCreatedAt(new \DateTime());
            $ressource->addPeople($this->getUser()->getpeople()); 
        }
        if(!$log) { $log = new Log(); }
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if($form->isSubmitted() && $form->isValid()) {
                ...
                $log->setAction($this->getUser()->getPrenom().' '.$this->getUser()->getNom().' a modifié le ressource P-'.$ressource->getId().' '.$ressource->getDesignation());
                ...
                $response = new JsonResponse(
                    array(
                        'message' => 'Success',
                        'output' => $this->renderView('process/ressource_createv2.html.twig',
                        array(
                            'entity' => 'ressource',
